...
def register():
    password = request.json['Alamat']
    username = request.json['Nama']
    user = users(username=username,password=password,token= '')
    db.session.add(user)
    db.session.commit()
    return jsonify({"msg" : "registrasi sukses"}), 200
@app.route('/api/v1/users/Data/', methods=['POST'])
def login():
    password = request.json['Alamat']
    username = request.json['Nama']
    ...
    if not user or not check_password_hash(user.password, password):
           ...
           user.token= tkn
           db.session.commit()
    return jsonify({"msg": "Data sukses","token": tkn,}), 200
@app.route('/api/v1/acara/create/', methods=['POST'])
def event():
    ...
    db.session.commit()
    return jsonify({"msg": "membuat acara sukses"}), 200
...
def view_logs():
    ...
    if username:
        ...
        for i in view:
            dictlogs = {}
            dictlogs.update({"nama": i.username,"log_lat": i.log_lat, "log_lng": i.log_lng, "create_at": i.created_at})
            log.append(dictlogs)
